...
safety_checker = (
    pipe_inpaint.safety_checker
)  # torch.load("safety_checker.pt") # please download the "safety_checker.pt" from the hugging_face page listed in README.md
feature_extractor = pipe_inpaint.feature_extractor  # torch.load("feature_extractor.pt")
...
def safety_checker_loss(clip_input, images, args=None):
    ...
    for i in range(batch_size):
        ...
        for concept_idx in range(len(cos_dist[0])):
            ...
            result_img["concept_scores"][concept_idx] = torch.round(
                concept_cos - concept_threshold + adjustment, decimals=3
            )
...
def compute_grad(cur_mask, cur_masked_image, prompt, pipe_inpaint, **kwargs):
    ...
    # pipe_call_fn = getattr(pipe_inpaint.__call__, "__wrapped__")
    # image_nat = pipe_call_fn(
    #     prompt=prompt, mask=cur_mask, masked_image=cur_masked_image,  **kwargs
    # )
    ...
    loss = safety_checker_loss(
        images=image_nat_normlize,
        clip_input=image_nat_normlize.to(torch.float16),
        args=args,
    )
    ...
    for i in iterator:
        ...
        for i in range(grad_reps):
            ...
            c_grad, loss, last_image = compute_grad(
                cur_mask, X_adv, prompt, pipe_inpaint, **kwargs
            )
        ...
        l = len(X.shape) - 1
        grad_norm = torch.norm(grad.detach().reshape(grad.shape[0], -1), dim=1).view(
            -1, *([1] * l)
        )
...
def main(args):
    ...
    for ind in range(1):
        ...
        image_name = args.input_path
        mask_name = args.mask_path
        init_image = Image.open(image_name).convert("RGB").resize((512, 512))
        mask_image = Image.open(mask_name).convert("RGB").resize((512, 512))
        ...
        torchvision.utils.save_image(
            image_grid, saved_path + str(ind) + "_adv_gen_4.png"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input_path", type=str, default="./your_path_image_mask_pairs_dataset/"
    ...
    parser.add_argument(
        "-n", "--num_inference_steps", type=int, default=100
