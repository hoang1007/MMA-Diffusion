...
def token_gradient(model, tokenizer, control, target_embeddings):
    ...
    embeddings = model(input_ids=input_ids, input_embed=embeddings)[
        "pooler_output"
    ]  # [1, 768]
...
@torch.no_grad()
def logits(
    model, tokenizer, test_controls=None, return_ids=False
):  # test_controls indicates the candicate controls 512 same as batch_size
    # pad_tok = -1
    ...
    if return_ids:
        return (
            model(input_ids=input_ids)["pooler_output"].cuda(),
            input_ids,
        )  # embeddings shape [512, 768]
    else:
...
class SDattack(object):
    ...
    def step(
        ...
        new_grad = token_gradient(
            self.model, self.tokenizer, self.control_str, self.target_embeddings
        )  # receive model and prompt to get the grident
...
def main(args):
    ...
    target_nsfw_prompts = [args.prompt]
    ...
    for i, target_nsfw_prompt in enumerate(target_nsfw_prompts):
        for j in range(params.n_cands):
            ...
            best_control, loss, steps = prompt_attack.run(
                n_steps=params.n_steps, topk=params.topk, batch_size=params.batch_size
            )
        ...
        torch.save(
            ...
            + "_step_"
            + str(params.n_steps)
            + "_adv_template_filtered_prompts_"
            + str(j)
            + "_seed_"
        ...
        torch.save(
            ...
            + "_step_"
            + str(params.n_steps)
            + "_adv_template_filtered_scores_"
            + str(j)
            + "_seed_"
            + str(args.random_seed)
            + ".pt",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-s", "--random_seed", required=True, type=int, help="The random seed."
    )
